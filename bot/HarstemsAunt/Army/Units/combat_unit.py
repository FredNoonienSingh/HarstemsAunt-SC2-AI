...
from bot.HarstemsAunt.dataclasses.TargetAllocation import Allocation
from bot.HarstemsAunt.common import MIN_SHIELD_AMOUNT, RANGE_BUFFER,\
    PROXIMITY, ALL_STRUCTURES,UNIT_LABEL_FONT_SIZE, logger
...
class CombatUnit:
    ...
    def __repr__(self) -> str:
        ...
        return "empty wrapper -> will be removed next tick"
    ...
    def is_safe(self) -> bool:
        """ Returns true is the current position is safe"""
        return self.bot.pathing.is_position_safe(grid=self.pathing_grid,
    ...
    @property
    def closest_friend(self) -> Optional[Unit]:
        """returns the closest friendly unit"""
        if self.friendlies_in_proximity:
            filtered_friends = self.friendlies_in_proximity.filter(
                lambda u: u.tag not in self.unit.passengers_tags and not u.tag == self.tag
            )
            return filtered_friends.closest_to(self.unit) if filtered_friends else None
    ...
    @property
    def closest_enemy(self) -> Optional[Unit]:
        """ returns the closet unit """
        if self.enemies_in_proximity:
            return self.enemies_in_proximity.closest_to(self.unit)

    @property
    def enemies_in_range(self) -> Units:
        """returns enemies in range"""
        if self.enemies_in_proximity and self.unit:
            return self.enemies_in_proximity.closer_than(self.unit.ground_range, self.unit)\
                .filter(lambda unit: unit.type_id not in ALL_STRUCTURES)
        return
    ...
    def in_attack_range_of(self) -> Units:
        ...
        """
        if self.enemies_in_proximity:
            return self.enemies_in_proximity.filter\
                (lambda e: e.distance_to(self.unit)< e.ground_range+RANGE_BUFFER)
        return None
    ...
    def potential_damage_given(self) -> float:
        ...
        if self.enemies_in_proximity:
            ...
            sorted_enemies = sorted(
                ...
                key=lambda enemy: self.unit.calculate_dps_vs_target(enemy),
                reverse=True
                )
    ...
    def can_survive_fleeing(self) -> bool:
        ...
        if self.potential_damage_taken:
            ...
            # probably spread out over multiple units
    ...
    def unit_label(self) -> str:
        ...
        if self.unit:
            ...
            return f"{self.unit.type_id}\nweapon cooldown: {self.unit.weapon_cooldown}\nfight status: {self.fight_status}\nin region: {self.region}\nbuffs: {self.unit.buffs}\nin range of {self.in_attack_range_of}"
        return None

    @property
    def cargo_size(self) -> int:
        """returns how much cargo space the unit needs """
        if self.unit:
            return self.unit.cargo_size

    def calculate_damage_vs_target(self, enemy:Unit) -> Optional[float]:
        if not self.unit:
            return None
        return self.unit.calculate_damage_vs_target(enemy)

    # TODO: breaks a bit on Bases !!! -> needs to be fixed
    def get_retreat_pos(self) -> Point2:
        ...
        if self.bot.debug:
            self.bot.debug_tools.draw_line_from_to(self.unit, next_region_center)
    ...
    async def engage(self, attack_target:Union[Unit, Point2]) -> None:
        ...
        if not attack_target:
            logger.error(f" NO TARGET ... for {self}")
            return
        ...
        if self.unit.weapon_cooldown <= 0.1:
    ...
    async def disengage(self, retreat_position: Point2) -> None:
        ...
        if not self.unit:
            if self.bot.debug:
                logger.warning("Unit not in existence")
            return
        self.bot.client.debug_text_world(self.unit_label,\
            self.unit, size=UNIT_LABEL_FONT_SIZE)
        move_to: Point2 = self.bot.pathing.find_path_next_point(
