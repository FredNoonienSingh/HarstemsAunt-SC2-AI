...
from .common import RANGE_BUFFER,PRIO_ATTACK_TARGET,UNIT_LABEL_FONT_SIZE,logger
...
class CombatFlyer(CombatUnit):
    """Base Class for flying Units """

    @property
    def ground_targets_in_proximity(self):
        return self.enemies_in_proximity.filter(lambda unit: not unit.is_flying)

    @property
    def air_targets_in_proximity(self):
        return self.enemies_in_proximity.filter(lambda unit: unit.is_flying)
