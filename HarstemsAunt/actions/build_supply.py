...
from utils.can_build import can_build_unit, can_build_structure

async def build_supply(bot: BotAI, build_pos) -> None:
    if not bot.can_afford(UnitTypeId.PYLON) or bot.supply_cap == 200:
        return
    if can_build_structure(bot,UnitTypeId.PYLON) and not bot.already_pending(UnitTypeId.PYLON) and bot.supply_left < 5 and len(bot.structures(UnitTypeId.NEXUS))>= 2:
